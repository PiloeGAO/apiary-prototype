...
class RunCreateModel(BaseModel):
    ...
    log: list[str] | SkipJsonSchema[None] = Field(
...
class TaskCreateModel(BaseModel):
    ...
    command: str
    status: Optional[int] = 0
    parents: list[str] | SkipJsonSchema[None] = Field(
        default=[], json_schema_extra=lambda x: x.pop("default")
    )
...
class TaskEditModel(TaskCreateModel):
    ...
    runs: list[RunModel] | SkipJsonSchema[None] = Field(
        default=[], json_schema_extra=lambda x: x.pop("default")
    )
...
class JobCreateModel(BaseModel):
    ...
    tasks: list[TaskCreateModel] | SkipJsonSchema[None] = Field(
        default=[], json_schema_extra=lambda x: x.pop("default")
    )
    tags: list[str] | SkipJsonSchema[None] = Field(
        default=[], json_schema_extra=lambda x: x.pop("default")
